          ...
          <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
            <div className="flex flex-wrap justify-between gap-3 p-4">
              ...
              <Link href="/admin/articles/new">
                <button className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-8 px-4 bg-[#f4f3f0] text-[#181511] text-sm font-medium leading-normal hover:bg-[#e8e6e1] transition-colors">
            ...
            <div className="px-4 py-3">
              <label className="flex flex-col min-w-40 h-12 w-full">
                <div className="flex w-full flex-1 items-stretch rounded-lg h-full">
                  <div className="text-[#887c63] flex border-none bg-[#f4f3f0] items-center justify-center pl-4 rounded-l-lg border-r-0">
                    ...
                    {filteredArticles.length > 0 ? (
                      filteredArticles.map((article) => (
                        <tr key={article.id} className="border-t border-t-[#e5e2dc]">
                          <td className="h-[72px] px-4 py-2 w-[400px] text-[#181511] text-sm font-normal leading-normal">{article.title}</td>
                          <td className="h-[72px] px-4 py-2 w-60 text-sm font-normal leading-normal">
                            <button className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-8 px-4 bg-[#f4f3f0] text-[#181511] text-sm font-medium leading-normal w-full">
                              <span className="truncate">{article.status === 'published' ? '已发布' : '草稿'}</span>
                            </button>
                          </td>
                          <td className="h-[72px] px-4 py-2 w-[400px] text-[#887c63] text-sm font-normal leading-normal">{article.publishDate}</td>
                      ...
                      <tr className="border-t border-t-[#e5e2dc]">
                        <td colSpan={4} className="h-[72px] px-4 py-2 text-center text-[#887c63] text-sm">没有找到匹配的文章</td>
                      </tr>
                    )}
                  </tbody>
                </table>
