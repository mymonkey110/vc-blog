  ...
  return (
    <div className="flex flex-1 justify-center px-4 sm:px-8 lg:px-40 py-5">
      <div className="flex max-w-[960px] flex-1 flex-col">
        <h2 className="px-2 sm:px-4 py-3 pt-5 title-1 leading-tight tracking-[-0.015em]">
          最新文章
        </h2>
        ...
        <div className="grid gap-6 sm:gap-8 p-2 sm:p-4">
          {articles.map((article, index) => (
            <article
              key={article.id}
              className="flex flex-col gap-4 py-4 border-b border-gray-100 last:border-b-0"
            >
              {/* 移动端：图片在上方，桌面端：图片在右侧 */}
              ...
              <div className="flex flex-col sm:flex-row items-start gap-4">
                <div className="flex flex-col gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-1">
                    ...
                    {/* 分类标签位置 - 暂时隐藏但保留位置 */}
                    <a
                      className="text-sm font-ui text-secondary-text hover:text-accent transition-colors"
                      href="#"
                    >
                      分类
                    </a>
